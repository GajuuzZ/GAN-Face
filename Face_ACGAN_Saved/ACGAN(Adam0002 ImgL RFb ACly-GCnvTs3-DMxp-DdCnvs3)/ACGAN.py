...
TRAIN_NAME = 'ACGAN(Adam0002 ImgL RFb ACly-GCnvTs3-DMxp-DdCnvs3)'
...
class ACGAN:
    ...
    def build_generator(self):
        model = Sequential()
        # Dense
        model.add(ly.Dense(128 * 10 * 8, activation="relu", input_dim=self.latent_dim))
        # Reshape to Image
        model.add(ly.Reshape((10, 8, 128)))
        # Reverse Conv_1
        model.add(ly.Conv2DTranspose(128, kernel_size=3, strides=2, padding='same'))
        model.add(ly.Activation("relu"))
        # Reverse Conv_2
        model.add(ly.Conv2DTranspose(64, kernel_size=3, strides=2, padding='same'))
        model.add(ly.Activation("relu"))
        # Reverse Conv_3
        model.add(ly.Conv2DTranspose(32, kernel_size=3, strides=2, padding='same'))
        model.add(ly.Activation("relu"))
        # Conv_3
        model.add(ly.Conv2D(self.input_shape[-1], kernel_size=3, padding='same'))
    ...
    def build_discriminator(self):
        model = Sequential()
        # Conv_1
        model.add(ly.Conv2D(16, kernel_size=3, strides=1, padding="same",
        ...
        # Conv_2
        model.add(ly.Conv2D(32, kernel_size=3, strides=1, padding="same"))
        model.add(ly.LeakyReLU(alpha=0.2))
        model.add(ly.Conv2D(32, kernel_size=3, strides=1, padding='same'))
        model.add(ly.LeakyReLU(alpha=0.2))
        model.add(ly.MaxPooling2D(pool_size=2))
        # Conv_3
        model.add(ly.Conv2D(64, kernel_size=3, strides=1, padding="same"))
        model.add(ly.LeakyReLU(alpha=0.2))
        model.add(ly.Conv2D(64, kernel_size=3, strides=1, padding='same'))
        model.add(ly.LeakyReLU(alpha=0.2))
        model.add(ly.MaxPooling2D(pool_size=2))
        # Conv_4
        model.add(ly.Conv2D(128, kernel_size=3, strides=1, padding="same"))
        model.add(ly.LeakyReLU(alpha=0.2))
        model.add(ly.Conv2D(128, kernel_size=3, strides=1, padding="same"))
        model.add(ly.LeakyReLU(alpha=0.2))
        # Flat.
    ...
    def train(self, batch_gen, epochs, sample_interval=50):
        ...
        for epoch in range(epochs):
            ...
            et = time.time() - st  # Count time only on train process.
        ...
        txt = txt + '\nTime per epoch : ' + time.strftime('%H:%M:%S', time.gmtime(total_time / epochs))
...
if __name__ == '__main__':
    ...
    gan.train(gen, epochs=100, sample_interval=5)
